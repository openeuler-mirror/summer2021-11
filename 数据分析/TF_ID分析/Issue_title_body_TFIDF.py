...
def get_tokens(text):
    ...
    # tokens = fool.cut(no_punctuation)
    # print(no_punctuation)
    no_punctuation = re.sub('[\u4e00-\u9fa5]', ' ', no_punctuation)
    # print(no_punctuation)
...
def main(text_list):
    ...
    for i, count in enumerate(countlist):
        ...
        index += 1
    oe_closed_data['title_tfidf'] = title_tfidf
    colums = ['community_name', 'issue_id', 'title_tfidf', 'title', 'body', 'issue_url', 'assignee', 'label', 'creator',
              'comments', 'created_at']

    oe_closed_data.to_csv('OpenEuler+src_title_tfidf_only_eng.csv', index=None, columns=colums)

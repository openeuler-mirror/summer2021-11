...
community_data = pd.read_csv('Raw_data/Src-Openeuler-所有社区的信息.csv')
community_name = community_data['full_name'][5452:]
count = 0
for name in community_name:
    ...
    if count % 50 == 0:
        ...
        columns = ['community_name', 'issue_id', 'title', 'body', 'issue_url', 'assignee', 'label', 'creator',
                   'comments',
                   'created_at']
        oe_issue_list.to_csv('Src-Openeuler_all_issue_data_2.csv', index=None, columns=columns)
        print('csv已保存！', count)
    count += 1
oe_issue_list = pd.DataFrame(
    {

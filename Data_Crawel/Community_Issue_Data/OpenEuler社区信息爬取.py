...
def get_allch():
    url = 'https://gitee.com/api/v5/orgs/openEuler/repos?type=all&page=1&per_page=100'
    ...
    for i in range(1, int(pagenum) + 1):
        url = 'https://gitee.com/api/v5/orgs/openEuler/repos?type=all&page=' + str(i) + '&per_page=100'
        response = requests.get(url=url)
        data = response.json()
    ...
    columns = ['name', 'full_name', 'description', 'forks_count', 'stargazers_count', 'watchers_count',
               'open_issues_count']
    community_data.to_csv('Raw_data/Openeuler-所有社区的信息.csv', columns=columns)
    print('csv已保存！')
